...
for i = 1: m
      ...
      Y(temp) = 1;      
      J =  J + (- Y'*log( sigmoid(Theta2* [1;sigmoid(Theta1* [1 X(i,:)]')] ))-(1-Y)'*log( 1-sigmoid(Theta2* [1;sigmoid(Theta1* [1 X(i,:)]')] ) ));
...
Theta1_grad  = (Delta_1 + lambda*tmp1)/m;
Theta2_grad  = (Delta_2+ lambda*tmp2)/m;

% X = [ones(m, 1) X];
% layer1 =  [ones(m, 1) sigmoid(X*Theta1')];
% sigmoid(layer1*Theta2') 
